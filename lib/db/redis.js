 ...
 *   subkeyid:${sub-aor}:${res-aor}:${event type}:${id}
 *   subkeydlg:${sub-aor}:${res-aor}:${event type}:${call-id}
 */
...
function makeSubStateKeyId(subscriber, resource, event, id) {
  return `subkeyid:${subscriber}:${resource}:${event}:${id}`;
}

function makeSubStateKeyDialog(subscriber, resource, event, callid) {
  return `subkeydlg:${subscriber}:${resource}:${event}:${callid}`;
}
...
class Db extends Emitter {
  ...
  addSubscription(obj, expiry) {
    ...
    const keyDialog = makeSubStateKeyDialog(obj.subscriber, obj.resource, obj.name, obj.callId);

    debug(`db#addSubscription ${key} with expiry ${expiry} for ${JSON.stringify(obj)}`);
    ...
    return new Promise((resolve, reject) => {
      ...
      if (obj.id) {
        const keyId = makeSubStateKeyId(obj.subscriber, obj.resource, obj.name, obj.id);
        multi
          .set(keyId, key)
          .expire(keyId, expiry);
      }
