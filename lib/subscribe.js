...
function refresh(req, res, dlg, subscription) {
  logger.info(req.event, 'subscribe#refresh');
  return db.removeSubscription(subscription)
    .then(() => {
      return db.addSubscription(subscription, req.expiry);
    })
    .then(() => {
      return clearSubscriptionTimer(dlg, subscription, true);
    })
    .then(() => {
      return startSubscriptionTimer(dlg, subscription, req.expiry);
    })
    .then(() => {
      return res.send(202);
    })
...
function notify(subscription, dlg, subscriptionState) {
  logger.info(subscription, 'subscribe#notify');
  db.getEventState(subscription.resource, subscription.name)
    ...
    .catch((err) => {
      logger.error(err, subscription, `subscribe#notify: error retrieving state: ${err}`);
    });
}
...
function clearSubscriptionTimer(dlg, subscription, andCancel) {
  const subs = dialogs.get(dlg.id);
  const arr = _.remove(subs, (s) => { return s.subscription.name === subscription.name; });
